...
// --- ADICIONAR POST (form de add.html envia para cá) ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nome"], $_POST["descricao"], $_POST["nota"])) {
    $nome = $_POST["nome"];
...
// --- FILTRO ---
$ordem = $_POST['ordem'] ?? 'padrao';

$sql = "SELECT * FROM blog";

if ($ordem == "decrescente") {
    $sql .= " ORDER BY nota DESC";
} elseif ($ordem == "crescente") {
    $sql .= " ORDER BY nota ASC";
} elseif ($ordem == "recente") {
...
<form method="post">
    <select name="ordem">
        <option value="padrao" <?= $ordem == 'padrao' ? 'selected' : '' ?>>Padrão</option>
        <option value="decrescente" <?= $ordem == 'decrescente' ? 'selected' : '' ?>>maior pra menor nota</option>
        <option value="crescente" <?= $ordem == 'crescente' ? 'selected' : '' ?>>menor pra maior nota</option>
        <option value="recente" <?= $ordem == 'recente' ? 'selected' : '' ?>>Recente</option>
    </select>
    <input type="submit" value="Filtrar">
</form>

<!-- Botão adicionar / logar -->
<div class="botao">
